...
nonisolated enum TripPayloadCodec {
    static let currentPayloadVersion = 7
    ...
    static func decodeMigrating(payload: Data, payloadVersion: Int) throws -> (trip: Trip, payload: Data, payloadVersion: Int) {
        ...
        case 6:
            let migrated = try TripPayloadMigrator.migrateV6Payload(payload)
            return (try decode(migrated), migrated, currentPayloadVersion)
        case 5:
            let migrated = try TripPayloadMigrator.migrateV5Payload(payload)
            return (try decode(migrated), migrated, currentPayloadVersion)
        case 4:
            let migrated = try TripPayloadMigrator.migrateV4Payload(payload)
            return (try decode(migrated), migrated, currentPayloadVersion)
        case 3:
            let migrated = try TripPayloadMigrator.migrateV3Payload(payload)
            return (try decode(migrated), migrated, currentPayloadVersion)
        case 2:
            let migrated = try TripPayloadMigrator.migrateV2Payload(payload)
            return (try decode(migrated), migrated, currentPayloadVersion)
        case 1:
